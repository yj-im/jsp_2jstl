...
<h2>상품 카테고리</h2>
	<!-- 3_parameter.jsp 의 forEach는 items 속성을 사용합니다.
		 자바의 향상for문과 동작 방식이 같습니다.(list에서는 대부분 begin end 보다 items 속성으로 반복함.)
		 여기서는 기존 for문의 index를 사용합니다 (begin end 형식만 참고)
...
<%
	...
	// 2. 파라미터가 없는 URL 일때, 전체 상품 출력하기	
	TblProductDao dao= TblProductDao.getInstance();
	List<Product> list = null;
	if(category==null)
			list=dao.selectAllProduct();	// 카테고리값 변경 직접 문자열값 변경
	else
